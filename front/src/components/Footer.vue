<script setup>
import Contacts from './Contacts.vue';
import Socials from './Socials.vue';
import { ref, watch } from 'vue';
import { } from 'vue-router'
let activePopup = ref(null);
const closePopup = () => {
  activePopup.value = null;
};
watch(activePopup, async (newVal) => {
  if (newVal) {
    document.body.style.overflow = 'hidden';
  } else {
    document.body.style.overflow = 'auto';
  }
});
</script>

<template>
  <FooterPopup :activeCategory="activePopup" @close="closePopup" />
  <footer class="footer">
    <div class="container">
      <div class="wrapper">
        <div class="logo-wrapper">
          <div class="logo">
            <a href="#">
              <img src="../assets/logo-hor-1.svg" />
            </a>
          </div>
          <nav class="footer__popup">
            <router-link class="footer__popup-item" to="/about">
              О компании
            </router-link>
            <router-link to="/politics" class="footer__popup-item">
              Публичная оферта
            </router-link>
            <router-link to="/warranty" class="footer__popup-item">
              Возврат / Гарантия
            </router-link>
            <router-link to="/shipment" class="footer__popup-item">
              Доставка и оплата
            </router-link>
          </nav>
        </div>
        <div class="contacts-wrapper">
          <ul class="social">
            <li class="social__item">
              <a class="social__item-link" href="https://www.facebook.com/favory.almaty/">
                <svg xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" viewBox="0 0 24 24" id="facebook" width="40px"
                  height="40px">
                  <path
                    d="M15.12,5.32H17V2.14A26.11,26.11,0,0,0,14.26,2C11.54,2,9.68,3.66,9.68,6.7V9.32H6.61v3.56H9.68V22h3.68V12.88h3.06l.46-3.56H13.36V7.05C13.36,6,13.64,5.32,15.12,5.32Z">
                  </path>
                </svg>
              </a>
            </li>
            <li class="social__item">
              <a class="social__item-link"
                href="https://api.whatsapp.com/send/?phone=77777778050&text=%D0%97%D0%B4%D1%80%D0%B0%D0%B2%D1%81%D1%82%D0%B2%D1%83%D0%B9%D1%82%D0%B5.+%D0%A0%D0%B0%D1%81%D1%81%D0%BA%D0%B0%D0%B6%D0%B8%D1%82%D0%B5+%D0%BF%D0%BE%D0%B4%D1%80%D0%BE%D0%B1%D0%BD%D0%B5%D0%B5+%D0%BF%D1%80%D0%BE+%D1%8E%D0%B2%D0%B5%D0%BB%D0%B8%D1%80%D0%BD%D1%8B%D0%B5+%D1%83%D0%BA%D1%80%D0%B0%D1%88%D0%B5%D0%BD%D0%B8%D1%8F+Favory&type=phone_number&app_absent=0">
                <svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" viewBox="0 0 24 24"
                  id="whatsapp" width="40px" height="40px">
                  <path
                    d="M16.6 14c-.2-.1-1.5-.7-1.7-.8-.2-.1-.4-.1-.6.1-.2.2-.6.8-.8 1-.1.2-.3.2-.5.1-.7-.3-1.4-.7-2-1.2-.5-.5-1-1.1-1.4-1.7-.1-.2 0-.4.1-.5.1-.1.2-.3.4-.4.1-.1.2-.3.2-.4.1-.1.1-.3 0-.4-.1-.1-.6-1.3-.8-1.8-.1-.7-.3-.7-.5-.7h-.5c-.2 0-.5.2-.6.3-.6.6-.9 1.3-.9 2.1.1.9.4 1.8 1 2.6 1.1 1.6 2.5 2.9 4.2 3.7.5.2.9.4 1.4.5.5.2 1 .2 1.6.1.7-.1 1.3-.6 1.7-1.2.2-.4.2-.8.1-1.2l-.4-.2m2.5-9.1C15.2 1 8.9 1 5 4.9c-3.2 3.2-3.8 8.1-1.6 12L2 22l5.3-1.4c1.5.8 3.1 1.2 4.7 1.2 5.5 0 9.9-4.4 9.9-9.9.1-2.6-1-5.1-2.8-7m-2.7 14c-1.3.8-2.8 1.3-4.4 1.3-1.5 0-2.9-.4-4.2-1.1l-.3-.2-3.1.8.8-3-.2-.3c-2.4-4-1.2-9 2.7-11.5S16.6 3.7 19 7.5c2.4 3.9 1.3 9-2.6 11.4">
                  </path>
                </svg>
              </a>
            </li>
            <li class="social__item">
              <a class="social__item-link" href="https://www.instagram.com/favory.kz/">
                <svg xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" viewBox="0 0 24 24" id="instagram"
                  width="40px" height="40px">
                  <path
                    d="M17.34,5.46h0a1.2,1.2,0,1,0,1.2,1.2A1.2,1.2,0,0,0,17.34,5.46Zm4.6,2.42a7.59,7.59,0,0,0-.46-2.43,4.94,4.94,0,0,0-1.16-1.77,4.7,4.7,0,0,0-1.77-1.15,7.3,7.3,0,0,0-2.43-.47C15.06,2,14.72,2,12,2s-3.06,0-4.12.06a7.3,7.3,0,0,0-2.43.47A4.78,4.78,0,0,0,3.68,3.68,4.7,4.7,0,0,0,2.53,5.45a7.3,7.3,0,0,0-.47,2.43C2,8.94,2,9.28,2,12s0,3.06.06,4.12a7.3,7.3,0,0,0,.47,2.43,4.7,4.7,0,0,0,1.15,1.77,4.78,4.78,0,0,0,1.77,1.15,7.3,7.3,0,0,0,2.43.47C8.94,22,9.28,22,12,22s3.06,0,4.12-.06a7.3,7.3,0,0,0,2.43-.47,4.7,4.7,0,0,0,1.77-1.15,4.85,4.85,0,0,0,1.16-1.77,7.59,7.59,0,0,0,.46-2.43c0-1.06.06-1.4.06-4.12S22,8.94,21.94,7.88ZM20.14,16a5.61,5.61,0,0,1-.34,1.86,3.06,3.06,0,0,1-.75,1.15,3.19,3.19,0,0,1-1.15.75,5.61,5.61,0,0,1-1.86.34c-1,.05-1.37.06-4,.06s-3,0-4-.06A5.73,5.73,0,0,1,6.1,19.8,3.27,3.27,0,0,1,5,19.05a3,3,0,0,1-.74-1.15A5.54,5.54,0,0,1,3.86,16c0-1-.06-1.37-.06-4s0-3,.06-4A5.54,5.54,0,0,1,4.21,6.1,3,3,0,0,1,5,5,3.14,3.14,0,0,1,6.1,4.2,5.73,5.73,0,0,1,8,3.86c1,0,1.37-.06,4-.06s3,0,4,.06a5.61,5.61,0,0,1,1.86.34A3.06,3.06,0,0,1,19.05,5,3.06,3.06,0,0,1,19.8,6.1,5.61,5.61,0,0,1,20.14,8c.05,1,.06,1.37.06,4S20.19,15,20.14,16ZM12,6.87A5.13,5.13,0,1,0,17.14,12,5.12,5.12,0,0,0,12,6.87Zm0,8.46A3.33,3.33,0,1,1,15.33,12,3.33,3.33,0,0,1,12,15.33Z">
                  </path>
                </svg>
              </a>
            </li>
          </ul>

          <ul class="contact">
            <li class="contact__item">
              <a class="contact__item__link" href="https://2gis.kz/almaty/firm/9429940000787107" target="_blank">
                <span>Казахстан, г. Алматы, ул. Назарбаева, 152 </span>
              </a>
            </li>
            <li class="contact__item">
              <a class="contact__item-link" href="tel:+77777007333">
                +7 777 700 73 33
              </a>
            </li>
            <li class="contact__item">
              <a class="contact__item-link" href="mailto:info@favory.kz">
                info@favory.kz
              </a>
            </li>
          </ul>
        </div>

        <nav class="footer__popup end">
          <router-link to="/keep" class="footer__popup-item">
            Хранение и уход
          </router-link>
          <router-link to="/table" class="footer__popup-item">
            Таблица размеров
          </router-link>
          <router-link to="/faq" class="footer__popup-item">
            Вопросы-ответы
          </router-link>
        </nav>
      </div>
    </div>
  </footer>
</template>
<style scoped>
.footer {
  padding: 28px 0 44px;
  color: #5c5c5c;
}

.footer__popup {
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-end;
  /* flex-wrap: wrap; */
  font-style: italic;
  gap: 5px;
  /* margin-top: 40px;  */
}

.footer__popup button {
  font-style: italic;
  font-size: 20px;
}

.footer__popup.end {
  margin-top: 80px;
  align-items: flex-end;
}

.logo-wrapper,
.footer__popup {
  flex: 0 0 25%;
}

.footer__popup-item {
  display: block;
  background: transparent;
  border: none;
  transition: 0.3s;
  color: #5c5c5c;
  /* margin-bottom: 10px; */
  font-style: italic;
  font-size: 20px;
}

.footer__popup-item:hover {
  /* opacity: 0.8; */
  color: #202525;
  /* background-color: var(--color-black);
  color: #fff; */
}

.footer__popup-item:last-child {
  /* margin-bottom: 0; */
}

.footer-title {
  margin-bottom: 15px;
  font-size: 18px;
}

.container {
  max-width: 1410px;
  width: 95%;
  margin: 0 auto;
}

.logo {
  margin-bottom: 15px;
}

.social__item path {
  fill: #5c5c5c;
  transition: all 0.3s;
}

.social__item rect {
  fill: #fff;
  transition: all 0.2s;
}

.social__item:hover rect {
  fill: #5c5c5c;
}

.social__item:hover path {
  fill: #000;
}

.wrapper {
  display: flex;
  /* align-items: center; */
  align-items: flex-start;
  justify-content: space-between;
}

.social {
  padding-top: 0px;
  display: flex;
  list-style: none;
  gap: 25px;
  justify-content: center;
  margin-bottom: 30px;
}

.contact {
  font-style: italic;
}

.contact__item {
  font-size: 22px;
  line-height: 1.2;
}

.mobile {
  padding: 11px 19px;
  text-transform: uppercase;
  font-size: 14px;
  transition: all 0.3s;
}

.contacts-wrapper {
  text-align: center;
}

.mobile:hover {
  background-color: var(--color-black);
  color: #fff;
}

@media (max-width: 1200px) {
  .wrapper {
    flex-wrap: wrap;
    /* justify-content: flex-start; */
  }

  .address {}
}

@media (max-width: 991px) {
  .logo {
    /* margin-bottom: 42px; */
  }

  .wrapper {
    flex-direction: column;
    gap: 20px;
    align-items: flex-start;
  }

  .contacts-wrapper {
    order: 3;
  }

  .footer-title {
    margin-bottom: 5px;
  }

  .address {
    text-align: left;
    justify-content: flex-start;
    margin-bottom: 26px;
  }

  .wrapper {
    gap: 0;
  }

  .wrapper .address span {
    text-align: left;
  }

  .mobile {
    display: none;
  }

  .footer__popup {
    margin-top: 10px;
    flex-direction: column;
    gap: 10px;
  }

  .social {
    margin-top: 30px;
    justify-content: flex-start;
    gap: 15px;
    margin-bottom: 0;
  }

  .contact__item {
    font-size: 18px;
    text-align: left;
  }

  .footer__popup.end {
    margin-top: 10px;
    align-items: flex-start;
  }

  .footer__popup-item {
    text-align: left;
  }
}
</style>
